...
vec3 getNormal(vec3 p)
{
	...
	return normalize(vec3(
		distScene(p + vec3(h, 0, 0)).x - distScene(p - vec3(h, 0, 0)).x,
		distScene(p + vec3(0, h, 0)).x - distScene(p - vec3(0, h, 0)).x,
		distScene(p + vec3(0, 0, h)).x - distScene(p - vec3(0, 0, h)).x));
}


void rayMarch(vec3 ro, vec3 rd, out int i, out float t, out float m)
{
	t = 0.0;
	m = -1.0;
...
float computeShadow(vec3 p, vec3 lightDir)
{
	vec3 ro = p - lightDir * 100.0;
	float t, m;
	int i;
	rayMarch(ro, lightDir, i, t, m);
...
vec3 computeColor(vec3 ro, vec3 rd)
{
	...
	if(m >= 0.0)
	{
		...
		vec3 p = ro + rd * t;
		// shadow
		float s = computeShadow(p, uLightDir);
